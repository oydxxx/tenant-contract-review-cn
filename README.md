...
```sh
npx -y skills add <GitHub-用户名>/<仓库名> -g --all
```

例如，仓库发布为 `example/tenant-contract-review-cn` 后：

```sh
npx -y skills add example/tenant-contract-review-cn -g --all
```
...
> 这条安装命令只在仓库已经公开可访问后可用。当前项目尚未创建 GitHub 远程；发布时请将示例中的占位地址替换为实际的 `<GitHub-用户名>/<仓库名>`。
